...
# 70 "../../VLCBlib_PIC/can18_can_2.c" 2
...
static DiagnosticVal * canGetDiagnostic(uint8_t index) {
    ...
    switch (index) {
        case 0x03:
            canDiagnostics[0x03].asUint = C1TRECU;
            break;
        case 0x04:
            canDiagnostics[0x04].asUint = getNumTxBuffersInUse();
            break;
        case 0x07:
...
static SendResult canSendMessage(Message * mp) {
    ...
    if (isEvent((uint8_t)(mp->opc))) {
        if (have(SERVICE_ID_CONSUME_OWN_EVENTS)) {
            ...
            if (temp > canDiagnostics[0x12].asUint) {
                canDiagnostics[0x12].asUint = temp;
            }
    ...
    if (temp > canDiagnostics[0x11].asUint) {
        canDiagnostics[0x11].asUint = temp;
    }
