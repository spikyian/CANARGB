...
typedef union PaletteIndex {
    struct PaletteIndexPair {
        uint8_t flashOnPaletteIndex : 4;
        uint8_t flashOffPaletteIndex : 4;
    } asNibbles;
    uint8_t asByte;
} PaletteIndex;

extern void updateLedRange(uint8_t start_ledno, uint8_t end_ledno, PaletteIndex colour);
extern void refreshString(void);
extern void initARGB(void);
...
void APP_testMode(void) {
    ...
    step = 0;
    while (TRUE) {
        if ((tickGet() - testTime.val) > (4 * 62500)) {
            testTime.val = tickGet();
            step++;
            if (step >= 6) step = 0;
            ...
            switch (step) {
                case 0:
                    updateLedRange(0, 255 -1, (PaletteIndex)((uint8_t)0x99));
                    break;
                case 1:
                    updateLedRange(0, 255 -1, (PaletteIndex)((uint8_t)0xAA));
                    break;
                case 2:
                    updateLedRange(0, 255 -1, (PaletteIndex)((uint8_t)0xBB));
                    break;
                case 3:
                    updateLedRange(0, 255 -1, (PaletteIndex)((uint8_t)0x00));
                    for (i=0,c=1; i<255; i++) {
                ...
                case 4:
                    ...
                    updateLedRange(240, 255 -1, (PaletteIndex)((uint8_t)0x0F));
                    break;
                case 5:
                    updateLedRange(0, 255 -1, (PaletteIndex)((uint8_t)0x00));
                    i=0;
        ...
        if (step == 5) {
            if ((tickGet() - subtestTime.val) > (62500/10)) {
                subtestTime.val = tickGet();
                updateLedRange(i,i, (PaletteIndex)((uint8_t)0xFF));
                i++;
            }
        }
    }
